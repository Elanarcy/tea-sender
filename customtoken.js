...
// ERC-20 ABI with transfer, balanceOf, and name functions
const tokenAbi = [
...
// Function to prompt user for token contract address and fetch token name
async function promptTokenAddress(provider, signer) {
  return new Promise(async (resolve) => {
    rl.question('Please enter the token contract address: ', async (answer) => {
      if (answer.startsWith('0x') && answer.length === 42) {
        console.log('Token contract address accepted.');
        const tokenContract = new ethers.Contract(answer, tokenAbi, signer);
        try {
          const tokenName = await tokenContract.name();
          console.log(`Token name: ${tokenName}`);
          resolve({ address: answer, name: tokenName, contract: tokenContract });
        } catch (error) {
          console.warn('Could not fetch token name (might not implement name()):', error.message);
          resolve({ address: answer, name: 'Unknown Token', contract: tokenContract });
        }
      } else {
        console.log('Invalid address. It should be a 40-character hex string with "0x" prefix (42 total). Try again.');
        resolve(await promptTokenAddress(provider, signer));
      }
    });
  });
}

// Function to prompt user for number of loops
function promptLoopCount() {
  return new Promise((resolve) => {
    rl.question('Please enter the number of loops (1-100): ', (answer) => {
      const num = parseInt(answer, 10);
      if (isNaN(num) || num < 1 || num > 100) {
        console.log('Invalid input. Please enter a number between 1 and 100.');
        resolve(promptLoopCount());
      } else {
        console.log(`Loop count accepted: ${num}`);
        resolve(num);
      }
    });
  });
...
// Function to send 10 tokens to a recipient address with retry logic
async function sendTokens(signer, tokenContract, tokenName, recipientAddress, amountToSendWei, gasReservedWei, maxRetries = 3) {
  let attempt = 0;
  while (attempt < maxRetries) {
    try {
      const balanceTea = await signer.provider.getBalance(signer.address);
      if (balanceTea < gasReservedWei) {
        throw new Error(`Insufficient TEA balance for gas: ${ethers.formatEther(balanceTea)} TEA (required: ${ethers.formatEther(gasReservedWei)} TEA)`);
      }

      const balanceTokens = await tokenContract.balanceOf(signer.address);
      if (balanceTokens < amountToSendWei) {
        throw new Error(`Insufficient ${tokenName} balance: ${ethers.formatUnits(balanceTokens, 18)} ${tokenName} (required: ${ethers.formatUnits(amountToSendWei, 18)} ${tokenName})`);
      }
      ...
      const tx = await tokenContract.transfer(recipientAddress, amountToSendWei, {
        gasPrice: gasPrice,
        gasLimit: gasLimit,
        nonce: nonce,
      });

      console.log(`Sending 10 ${tokenName} to ${recipientAddress} (Attempt ${attempt + 1}/${maxRetries})`);
      console.log(`Transaction hash: ${explorerBaseUrl}${tx.hash}`);
      await tx.wait();
      console.log(`Transaction confirmed for ${recipientAddress}`);
      return true;
    } catch (error) {
      attempt++;
...
// Main function to process transactions with user-defined inputs
async function runBot() {
  const privateKey = await promptPrivateKey();
  const provider = new ethers.JsonRpcProvider(rpcUrl);
  const signer = new ethers.Wallet(privateKey, provider);
  
  const { address: tokenAddress, name: tokenName, contract: tokenContract } = await promptTokenAddress(provider, signer);
  const totalIterations = await promptLoopCount();

  console.log(`Sender address: ${signer.address}`);
  const balanceTea = await provider.getBalance(signer.address);
  const balanceTokens = await tokenContract.balanceOf(signer.address);
  console.log(`Sender TEA balance: ${ethers.formatEther(balanceTea)} TEA`);
  console.log(`Sender ${tokenName} balance: ${ethers.formatUnits(balanceTokens, 18)} ${tokenName}`);

  const amountToSendWei = ethers.parseUnits('10', 18); // 10 tokens (assuming 18 decimals)
  const gasReservedWei = ethers.parseUnits('0.1', 'ether'); // 0.1 TEA gas fee per transaction
  const totalTransactions = recipientAddresses.length * totalIterations;
  const totalRequiredTeaWei = gasReservedWei * BigInt(totalTransactions);
  const totalRequiredTokensWei = amountToSendWei * BigInt(totalTransactions);

  console.log(`Total required: ${ethers.formatEther(totalRequiredTeaWei)} TEA (gas) + ${ethers.formatUnits(totalRequiredTokensWei, 18)} ${tokenName} for ${totalTransactions} transactions (${recipientAddresses.length} addresses x ${totalIterations} iterations)`);

  if (balanceTea < totalRequiredTeaWei) {
    console.error(`Insufficient TEA funds for gas. Required: ${ethers.formatEther(totalRequiredTeaWei)} TEA, Available: ${ethers.formatEther(balanceTea)} TEA`);
    rl.close();
    return;
  }
  if (balanceTokens < totalRequiredTokensWei) {
    console.error(`Insufficient ${tokenName} funds. Required: ${ethers.formatUnits(totalRequiredTokensWei, 18)} ${tokenName}, Available: ${ethers.formatUnits(balanceTokens, 18)} ${tokenName}`);
    rl.close();
    return;
  }

  const batchSize = 5; // Process 5 addresses at a time within each iteration
  for (let iteration = 1; iteration <= totalIterations; iteration++) {
    console.log(`Starting iteration ${iteration}/${totalIterations}`);
    for (let i = 0; i < recipientAddresses.length; i += batchSize) {
      const batch = recipientAddresses.slice(i, i + batchSize);
      for (const address of batch) {
        await sendTokens(signer, tokenContract, tokenName, address, amountToSendWei, gasReservedWei);
      }
